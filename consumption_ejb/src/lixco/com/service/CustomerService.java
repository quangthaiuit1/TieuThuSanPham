...
public class CustomerService implements ICustomerService {
	...
	public int search(String json, PagingInfo page, List<Customer> list) {
		...
		long totalPage = 0;
		try {/*
				 * {customer_info:{customer_code:'',customer_name:'',tax_code:'',
				 * phone_number:'',customer_types_id:0}, page:{page_index:0,
				 * page_size:0}}
				 */
	...
	public int complete(String text, List<Customer> list) {
		...
		try {
			if (text != null && !"".equals(text)) {
				...
				dis.getExpressions().add(cb.like(cb.function("replace", String.class,
						cb.lower(root.get("customer_name")), cb.literal("đ"), cb.literal("d")), "%" + text + "%"));
				cq.select(cb.construct(Customer.class, root.get("id"), root.get("customer_code"),
