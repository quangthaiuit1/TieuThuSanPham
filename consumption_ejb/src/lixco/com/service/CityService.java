...
public class CityService implements ICityService{
	...
	public int search(String json, PagingInfo page, List<City> list) {
		...
		long totalRow=0;
		long totalPage=0;
		try{/*{ city_info:{city_code:'',city_name:'',country_id:0,area_id:0}, page:{page_index:0, page_size:0}}*/
	...
	public int findLike(String text, int size, List<City> list) {
		...
		try{
			...
			dis.getExpressions().add(cb.like(cb.function("replace", String.class, cb.lower(root.get("city_name")),cb.literal("đ"),cb.literal("d")), paramLike));
...
//			logger.error("CityService.selectByCode:"+e.getMessage(),e);
	...
	public int complete(String text, List<City> list) {
		...
		try{
			...
			dis.getExpressions().add(cb.like(cb.function("replace", String.class, cb.lower(root.get("city_name")),cb.literal("đ"),cb.literal("d")), "%"+text+"%"));
			cq.select(cb.construct(City.class, root.get("id"),root.get("city_code"),root.get("city_name"))).where(dis);
