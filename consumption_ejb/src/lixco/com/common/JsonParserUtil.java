...
public class JsonParserUtil {
	...
	public static HolderParser getValueString(JsonElement json, String elementName,StringBuilder messages) {
		...
		try {
			...
			if (j != null) {
				...
				holder.setValue(r);
				if("".equals(r))
					messages.append(elementName+" is empty ?");
			} else {
				holder.setErr(-2);
		...
		try {
			...
			if (j != null) {
				...
				if(r.doubleValue()==0.0){
					message.append(elementName+" is zero ?");
				}
			} else {
				holder.setErr(-2);
				holder.setValue(0);
	...
	public static HolderParser getValueObject(JsonElement json,String elementName,StringBuilder messages,Type type ){
		// holder container result
		HolderParser holder = new HolderParser();
		try{
			JsonElement j=json.getAsJsonObject().getAsJsonObject(elementName);
			if(j != null){
				Object l=JsonParserUtil.getGson().fromJson(j,type);
				holder.setErr(0);
				holder.setValue(l);
			}else{
				holder.setErr(-2);
